...
export function CardProduct({ product }: { product: Product.Mini }) {
  ...
  const { user } = useUser();
  const { onOpenNeedAuth } = useApp();

  return (
    <li className="list-none group rounded-md mr-3 shadow-md transform translate-x-0 translate-y-0 translate-z-0 flex-shrink-0 w-[225px] max-sm:w-[185px] bg-custom-secondaryBrand h-fit" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <div className="relative h-64 max-sm:h-48 shadow-[inset_0_0_0_0.2rem_var(--detail-medium-contrast)] rounded-[1.8rem] text-[4rem] font-semibold flex items-center justify-center select-none">
        ...
        <div className={`absolute z-20 top-4 gap-2 right-1.5 ${addingItem === product.id ? "flex" : "hidden group-hover:flex"} flex-col text-2xl max-sm:flex`}>
          <button className={`w-10 h-10 rounded-full shadow-md bg-custom-accentColor duration-200 ease-linear ${addingItem === product.id ? "opacity-30" : "opacity-80"} flex items-center justify-center text-custom-textColor font-medium hover-no-opacity cursor-pointer max-sm:flex`} onClick={() => (user ? AddItemToCart({ product_id: product.id, quantity: 1, variation_id: product.color.sizes[0].id }) : onOpenNeedAuth())}>
        ...
        <Image className={`rounded-md hover-effect ${isHovered && "z-10"}`} src={product.color.images[0]} alt={`Image 1 of ${product.title}`} fill />
        {product.color.images[1] && <Image className={`rounded-md duration-500 ease-linear ${isHovered && "z-10 opacity-0"}`} src={product.color.images[1]} alt={`Image 2 of ${product.title}`} fill />}
      </div>
