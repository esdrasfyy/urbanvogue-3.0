...
import { InputCheckboxPropsI } from "@/app/components/ui/inputs/types/inputs";

export function InputCheckbox(data: InputCheckboxPropsI) {
  return (
    <>
    <div className={`flex w-fit container items-center ${data.classname}`}>
      <input type="checkbox" className="cursor-pointer relative m-auto w-[18px] h-[18px] " id="checkbox" style={{ display: "none" }} disabled={data.disabled}/>
      ...
      <label htmlFor="checkbox" className="flex flex-wrap w-full items-center cursor-pointer ml-5 max-sm:text-sm">{data.children}</label>
    </div>
    </>
